...
auto
rgb_to_lab(std::uint8_t r, std::uint8_t g, std::uint8_t b) -> Lab
{
    ...
    double fx =
        (x / xn > 0.008856) ? lab_cbrt(x / xn) : ((7.787 * x / xn) + (16.0 / 116.0));
    double fy =
        (y / yn > 0.008856) ? lab_cbrt(y / yn) : ((7.787 * y / yn) + (16.0 / 116.0));
    double fz =
        (z / zn > 0.008856) ? lab_cbrt(z / zn) : ((7.787 * z / zn) + (16.0 / 116.0));
    ...
    for (int r = 0; r < 6; r++)
        for (int g = 0; g < 6; g++)
            for (int b = 0; b < 6; b++)
            {
                ...
                table.at(idx)[0] =
                    static_cast<std::uint8_t>(r == 0 ? 0 : 95 + ((r - 1) * 40));
                table.at(idx)[1] =
                    static_cast<std::uint8_t>(g == 0 ? 0 : 95 + ((g - 1) * 40));
                table.at(idx)[2] =
                    static_cast<std::uint8_t>(b == 0 ? 0 : 95 + ((b - 1) * 40));
            }

    for (int i = 0; i < 24; i++)
    {
        int  idx      = 232 + i;
        auto val      = static_cast<std::uint8_t>(8 + (i * 10));
...
{
    std::array<std::array<std::uint8_t, 3>, 16> table{};
    table[0]  = {0, 0, 0};
    table[1]  = {128, 0, 0};
    table[2]  = {0, 128, 0};
    table[3]  = {128, 128, 0};
...
main() -> int
{
    ...
    std::array<std::uint8_t, TTY16_MAP_SIZE> tty16_map{};
    ...
    for (std::size_t qr = 0; qr < TTY16_QUANT_LEVEL; qr++)
        for (std::size_t qg = 0; qg < TTY16_QUANT_LEVEL; qg++)
            for (std::size_t qb = 0; qb < TTY16_QUANT_LEVEL; qb++)
            {
                ...
                Lab input_lab =
                    rgb_to_lab(static_cast<std::uint8_t>(r), static_cast<std::uint8_t>(g),
                ...
                tty16_map.at((qr * TTY16_QUANT_LEVEL * TTY16_QUANT_LEVEL) +
                    ...
                    static_cast<std::uint8_t>(ansi_code);
            }

    std::array<std::uint8_t, TTY256_MAP_SIZE> tty256_map{};
        ...
        for (std::size_t qg = 0; qg < TTY256_QUANT_LEVEL; qg++)
            for (std::size_t qb = 0; qb < TTY256_QUANT_LEVEL; qb++)
            {
                ...
                tty256_map.at((qr * TTY256_QUANT_LEVEL * TTY256_QUANT_LEVEL) +
                    ...
                    static_cast<std::uint8_t>(best_idx);
            }

    std::ofstream ofs{"include/tty_maps.h"};
    if (!ofs) return 1;

    ofs << std::format("// SPDX-License-Identifier: MIT\n");
    ofs << std::format("// Copyright (c) 2026 锯条\n");
    ofs << std::format("\n");
    ofs << std::format("#pragma once\n");
    ofs << std::format("\n");
    ofs << std::format("#include <array>\n");
    ofs << std::format("#include <cstddef>\n");
    ofs << std::format("#include <cstdint>\n");
    ofs << std::format("\n");
    ...
    {
        if (i % 16 == 0) ofs << std::format("    ");
        ofs << std::format("{}, ", static_cast<int>(tty16_map.at(i)));
        if (i % 16 == 15) ofs << std::format("\n");
    }
    ofs << std::format("}}}};\n");
    ofs << std::format("\n");
